...
- 실제 연산이 일어나게 되는 경우 전부 기본형 타입으로 연산 된다.
  ...
  | **실수형** | float      | 4 byte | (3.4 X 10^-38) ~ (3.4 X 10^38) 의 근사값   | 0.0F     |
  |            | **double** | 8 byte | (1.7 X 10^-308) ~ (1.7 X 10^308) 의 근사값 | 0.0      |
  | **문자형** | char       | 2 byte | 0 ~ 2^16 -1                                | '\u0000' |
...
- 가독성에 따라서 사용하면 어떤 타입인지 바로 알 수 있는 경우 사용하면 좋다. 하지만 가독성이 떨어질 수 있기에 주의 해야한다.
